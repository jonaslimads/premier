...
CREATE FUNCTION `GET_RANDOM_PLATFORM_EVENT_AGGREGATE_ID`(`range_start` BIGINT UNSIGNED, `range_end` BIGINT UNSIGNED) RETURNS BIGINT(20) UNSIGNED
READS SQL DATA
BEGIN
    DECLARE rnd BIGINT UNSIGNED;
    DECLARE i BIGINT UNSIGNED;
    ...
    r: REPEAT
        SET rnd = FLOOR(range_start + RAND() * (range_end - range_start));
        UNTIL NOT EXISTS( SELECT 1 FROM `platform_event` WHERE aggregate_id = rnd )
    END REPEAT r;
...
CREATE TRIGGER `UPDATE_GLOBAL_SEQUENCE_ON_PLATFORM_EVENT`
BEFORE INSERT
ON `platform_event` FOR EACH ROW
BEGIN
    DECLARE global_sequence BIGINT UNSIGNED;
...
-- CREATE TABLE internal_platform_view (
--     view_id VARCHAR(255) NOT NULL,
--     version BIGINT CHECK (version >= 0),
--     payload JSON NOT NULL,
--     CONSTRAINT PRIMARY KEY (view_id)
...
-- CREATE TABLE internal_product_view (
--     view_id VARCHAR(255) NOT NULL,
--     version BIGINT CHECK (version >= 0),
--     payload JSON NOT NULL,
--     CONSTRAINT PRIMARY KEY (view_id)
...
-- CREATE TABLE published_product_view (
--     view_id VARCHAR(255) NOT NULL,
--     version BIGINT CHECK (version >= 0),
--     payload JSON NOT NULL,
--     CONSTRAINT PRIMARY KEY (view_id)
